...
`analyze-mta-od-data one-seat-rides --markdown-out RESULTS.md`, not
hand-written. A `pre-commit` hook (`check-results-md`, see
`.pre-commit-config.yaml`) checks it's still up to date on every commit that
touches `src/mta_od_data/analyze.py` or `RESULTS.md` itself, so it can't
drift out of sync—see [Development](#development).
...
## The `analyze-mta-od-data` CLI

`src/mta_od_data/` is an installable package (`duckdb`/`typer` as its only
runtime dependencies); `uv sync` installs it and its `analyze-mta-od-data`
entry point into the project's `.venv`. Two subcommands:
...
```
uv run analyze-mta-od-data prepare-data
```
...
```
uv run analyze-mta-od-data one-seat-rides --routes B,D,N,Q,R \
...
installs the `git` hooks in `.pre-commit-config.yaml`: `ruff`, `ty`, and
`pyrefly`, plus a local `check-results-md` hook that runs
`uv run pytest tests/test_results_md.py` on every commit touching
`src/mta_od_data/analyze.py`, `tests/test_results_md.py`, or `RESULTS.md`.
That test reruns `analyze-mta-od-data one-seat-rides` into a scratch file
and diffs it against the committed `RESULTS.md`. Check-only, like the other
hooks (`ruff format --check`, `ty`, `pyrefly-check`): it never rewrites
...
(e.g. a fresh clone before running `analyze-mta-od-data prepare-data`)—the
same skip applies to a plain `uv run pytest`, since the real OD data is
