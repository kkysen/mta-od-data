...
See [`src/mta_od_data/analyze/one_seat_rides.md`](src/mta_od_data/analyze/one_seat_rides.md)
for the current output (headline numbers plus the top 25 origin/destination
pairs and top 25 destination stations) — generated by `mta-od-data analyze
one-seat-rides --markdown-out src/mta_od_data/analyze/one_seat_rides.md`,
not hand-written. A `pre-commit` hook (`pytest`, see
[`.pre-commit-config.yaml`](.pre-commit-config.yaml)) checks it's still up
to date on every commit, so it can't drift out of sync—see
...
[`one_seat_rides.md`](src/mta_od_data/analyze/one_seat_rides.md)).
