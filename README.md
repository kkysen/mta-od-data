...
`scripts/02_analyze.py --markdown-out RESULTS.md`, not hand-written. A
`pre-commit` hook (`check-results-md`, see `.pre-commit-config.yaml`) checks
it's still up to date on every commit that touches `scripts/02_analyze.py`
or `RESULTS.md` itself, so it can't drift out of sync—see
...
`pyrefly`, plus a local `check-results-md` hook
(`scripts/check_results_md.py`) that reruns `scripts/02_analyze.py` into a
scratch file and diffs it against the committed `RESULTS.md` on every commit
touching `scripts/02_analyze.py`, `scripts/check_results_md.py`, or
`RESULTS.md`. Check-only, like the other hooks (`ruff format --check`, `ty`,
`pyrefly-check`): it never rewrites `RESULTS.md` itself, it just fails with
a diff and the regen command to run by hand. If `data/mta_od.parquet` isn't
present (e.g. a fresh clone before running `scripts/01_prepare_data.py`),
the hook skips instead of blocking the commit.
