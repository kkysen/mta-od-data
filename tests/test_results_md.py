"""Snapshot test: RESULTS.md must match a fresh `analyze-mta-od-data` run.
...
committed) and only exists locally after `analyze-mta-od-data prepare-data`,
so this can't run in CI.
"""
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "(run `analyze-mta-od-data prepare-data` first)"
    ),
)
